# -*- coding: utf-8 -*-
"""
Created on Wed Mar 12 18:20:05 2025
...
def districting_heuristic(G, obj_type='cut_edges', enumeration_limit=10):
    ...
    G._size = math.floor(G._k / 2)   
    districts = enumerate_top_districts(G, obj_type=obj_type, enumeration_limit=enumeration_limit)
    partial_plans = [[district, list(set(G.nodes) - set(district))] for district in districts]
    plans = list()
    
    while partial_plans:
        partial_plan = partial_plans.pop()
        finished_clusters = 0
        for cluster in partial_plan:
            if len(cluster) == G._size:
                print("Finished cluster #", finished_clusters)
                finished_clusters += 1
                continue
            
            H = G.subgraph(cluster).copy()
            H._L = G._L
            H._U = G._U
            H._k = G._size
            H._root = None
            H._size = math.floor(G._size / 2)
        
            for i in H.nodes:
                if not H.nodes[i].get('boundary_node', False):
            ...
            for district in districts:
                new_cluster = [district, list(set(cluster) - set(district))]
                partial_plan = [new_cluster if c == cluster else c for c in partial_plan]
        
                if finished_clusters == 1:
