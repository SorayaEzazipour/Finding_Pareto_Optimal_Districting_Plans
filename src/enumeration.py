...
def enumerate_districts(G, L, U, k, root=None):
    ...
    m.addConstr(sum(G.nodes[i]['TOTPOP'] * x[i,0] for i in G.nodes) >= L)
    m.addConstr(sum(G.nodes[i]['TOTPOP'] * x[i,0] for i in G.nodes) <= U)
    m.addConstr(sum(G.nodes[i]['TOTPOP'] * x[i,1] for i in G.nodes) >= (k-1)*L)
    m.addConstr(sum(G.nodes[i]['TOTPOP'] * x[i,1] for i in G.nodes) <= (k-1)*U)
    ...
    m.addConstrs(sum(f[j,i] - f[i,j] for j in G.neighbors(i)) == x[i,0] for i in G.nodes if i != root)
    m.addConstrs(sum(f[j,i] for j in G.neighbors(i)) <= M * x[i,0] for i in G.nodes if i != root)
    m.addConstr(sum(f[j,root] for j in G.neighbors(root)) == 0)

    dist = nx.shortest_path_length(G, source=root)
    m.setObjective(sum(dist[i] * dist[i] * math.ceil(G.nodes[i]['TOTPOP']/1000) * x[i,0] for i in G.nodes), GRB.MINIMIZE)
