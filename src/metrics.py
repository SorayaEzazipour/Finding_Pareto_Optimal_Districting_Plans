...
def average_inverse_Polsby_Popper(G, districts, verbose=False):
    label = { i : j for j in range(len(districts)) for i in districts[j] }
    if verbose:
        print("\nDistrict Polsby-Popper scores:")
        for p in range(len(districts)):
            print(p, round(1/Polsby_Popper(G, districts[p], label),4) )
    return sum( 1/Polsby_Popper(G, district, label) for district in districts ) / len(districts)
...
# given two lists of objective values for two plans 
#   checks if plan1 dominates plan2
# senses is a list of the objective senses (min/max)
def dominates(plan1_scores, plan2_scores, senses):
    assert len(plan1_scores) == len(plan1_scores) 
    assert len(plan1_scores) == len(senses)
    
    # first, check if plan1_scores is at least as good in all entries
    for i in range(len(plan1_scores)):
        if senses[i] == 'min' and plan1_scores[i] > plan2_scores[i]:
            return False
        elif senses[i] == 'max' and plan1_scores[i] < plan2_scores[i]:
            return False
    if plan1_scores == plan2_scores:
        return False
