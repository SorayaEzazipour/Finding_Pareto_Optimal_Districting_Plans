...
def add_perimeter_objective(m, DG):
    ...
    m.setObjective( ep + sum( DG.edges[u,v]['shared_perim'] * m._y[u,v,j] for u,v in DG.edges for j in range(len(m._sizes)) ), GRB.MINIMIZE )
    m.update()
...
def add_inverse_Polsby_Popper_objective(m, DG):
    ...
    m.setObjective( ( 1.0 /len(m._sizes) ) * sum( z[j] for j in range(len(m._sizes)) ), GRB.MINIMIZE )
    ...
    m.addConstrs( A[j] == sum( DG.nodes[i]['area'] * m._x[i,j] for i in DG.nodes ) for j in range(len(m._sizes)) )

    # Add constraints on perimeters P[j]
    for j in range(len(m._sizes)):
        m.addConstr(P[j] == sum( DG.edges[u,v]['shared_perim'] * m._y[u,v,j] for u,v in DG.edges)
                + sum( DG.nodes[i]['boundary_perim'] * m._x[i,j] for i in DG.nodes if DG.nodes[i]['boundary_node'] ) )     
    m.update()
...
def add_average_Polsby_Popper_objective(m, DG):
    ...
    m.setObjective( ( 1.0 / len(m._sizes) ) * sum( (1/(2**(i+1))) * b[i,j] for j in range(len(m._sizes)) for i in range(num_digits) ), GRB.MAXIMIZE )
   ...
   # Add constraints on perimeters P[j]
    ...
    m.addConstrs( 1 >= sum( (1/(2**(i+1))) * w[i,j] for i in range(num_digits) ) for j in range(len(m._sizes)) )
...
def add_average_Schwartzberg_objective(m, DG):
    ...
    # objective is to minimize average of inverse Polsby-Popper scores
    m.setObjective( ( 1.0 / len(m._sizes) ) * sum( s[j] for j in range(len(m._sizes)) ), GRB.MINIMIZE )
    ...
    m.addConstrs( z[j] <= 1 + (big_M-1) * sum(2**(-(1 + t)) * b[t,j] for t in range(num_digits)) for j in range(len(m._sizes)) )
    m.addConstrs( 1 + (big_M-1) * sum(2**(-(1 + t)) * b[t,j] * b[t,j] for t in range(num_digits)) <= s[j]**2 for j in range(len(m._sizes)))
    m.update()

def add_bottleneck_Polsby_Popper_objective(m, DG):
